...
export function ResourceCard({ type = "pdf", title, meta = "", action = "Download", onAction, className }: ResourceCardProps) {
  ...
  return (
    <div
      className={cn(
        "reveal flex items-center gap-4 rounded-md border border-hair bg-card p-4 shadow-sm transition-shadow duration-220 ease-[cubic-bezier(0.4,0,0.2,1)] hover:shadow-md",
        className,
      )}
    >
